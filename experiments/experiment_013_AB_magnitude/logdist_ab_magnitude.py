...
def fit_logdist(
    ...
    lmin = (4.54 + 5.0 * np.log10(1.0 + df["zhelio"].to_numpy()) + df["kcor_j"].to_numpy() + df["extinction_j"].to_numpy() + 10.0 - 2.5 * np.log10(2.0 * math.pi) + 5.0 * np.log10(d_H) - mag_high) / 5.0
    lmax = (4.54 + 5.0 * np.log10(1.0 + df["zhelio"].to_numpy()) + df["kcor_j"].to_numpy() + df["extinction_j"].to_numpy() + 10.0 - 2.5 * np.log10(2.0 * math.pi) + 5.0 * np.log10(d_H) - mag_low) / 5.0
    ...
    df['logdist_pred'] = 0. #np.log10(d_z / d_H_pred)
...
def main():
    try:
        ...
        for survey in SURVEY_LIST:
            ...
            logdist_output_filepath = os.path.join(ROOT_PATH, f'experiments/experiment_013_AB_magnitude/{survey.lower()}.csv')
