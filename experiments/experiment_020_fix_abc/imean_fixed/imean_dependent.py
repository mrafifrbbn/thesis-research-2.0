...
def FP_func(params, logdists, z_obs, r, s, i, err_r, err_s, err_i, Sn, smin, lmin, lmax, C_m, sumgals=True, chi_squared_only=False, use_full_fn=True, c=-0.2):
...
def fit_FP(
    ...
    while not is_converged:
        ...
        chi_squared = Sn * FP_func(FPparams.x, 0.0, df["z_cmb"].to_numpy(), df["r_true"].to_numpy(), df["s"].to_numpy(), df["i"].to_numpy(), df["er"].to_numpy(), df["es"].to_numpy(), df["ei"].to_numpy(), Sn, smin, df["lmin"].to_numpy(), df["lmax"].to_numpy(), df["C_m"].to_numpy(), sumgals=False, chi_squared_only=True, c=c_value)[0]
...
def main():
    try:
        ...
        param_boundaries = [(a_, a_), (b_, b_), (-0.2, 0.4), (2.1, 2.5), (3.0, 3.5), (0.0, 0.06), (0.20, 0.45), (0.1, 0.25)]
        ...
        for survey in SURVEY_LIST:
            ...
            logdist_output_filepath = f"/Users/mrafifrbbn/Documents/thesis/thesis-research-2.0/experiments/experiment_020_fix_abc/imean_fixed/{survey.lower()}.csv"
        ...
        filepath_ = f"/Users/mrafifrbbn/Documents/thesis/thesis-research-2.0/experiments/experiment_020_fix_abc/imean_fixed/fp_fit.csv"
        pd.DataFrame(FP_params, columns=FP_columns, index=SURVEY_LIST).to_csv(filepath_)
