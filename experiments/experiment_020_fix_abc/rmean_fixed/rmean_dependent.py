...
def FN_func(FPparams, zobs, er, es, ei, lmin, lmax, smin, a, b):
...
def FP_func(params, logdists, z_obs, r, s, i, err_r, err_s, err_i, Sn, smin, lmin, lmax, C_m, sumgals=True, chi_squared_only=False, use_full_fn=True, a=1.5, b=-0.9, c=-0.2):
    ...
    # Calculate full f_n
    if use_full_fn:
        FN = FN_func(params, z_obs, err_r, err_s, err_i, lmin, lmax, smin, a, b) + np.log(C_m)
...
def fit_FP(
    ...
    while not is_converged:
        ...
        chi_squared = Sn * FP_func(FPparams.x, 0.0, df["z_cmb"].to_numpy(), df["r_true"].to_numpy(), df["s"].to_numpy(), df["i"].to_numpy(), df["er"].to_numpy(), df["es"].to_numpy(), df["ei"].to_numpy(), Sn, smin, df["lmin"].to_numpy(), df["lmax"].to_numpy(), df["C_m"].to_numpy(), sumgals=False, chi_squared_only=True, a=a_value, b=b_value, c=c_value)[0]
...
def sample_likelihood(df: pd.DataFrame,
    ...
    def log_probability(theta, logdists, z_obs, r, s, i, err_r, err_s, err_i, Sn, smin, lmin, lmax, C_m, sumgals=True, chi_squared_only=False, use_full_fn=True, a_value=1.5, b_value=-0.9, c_value=-0.2):
        lp = log_prior(theta)
        if not np.isfinite(lp):
            return -np.inf
        else:
            return lp - FP_func(theta, logdists, z_obs, r, s, i, err_r, err_s, err_i, Sn, smin, lmin, lmax, C_m, sumgals, chi_squared_only, use_full_fn, a_value, b_value, c_value)
    ...
    sampler = emcee.EnsembleSampler(
        nwalkers, ndim, log_probability, args=(0.0, z, r, s, i, dr, ds, di, Sn, smin, lmin, lmax, C_m, True, False, USE_FULL_FN, a_value, b_value, c_value)
    )
...
def main():
    ...
    a_ = FPparams["a"]
    b_ = FPparams["b"]
    c_ = FPparams["rmean"] - FPparams["a"] * FPparams["smean"] - FPparams["b"] * FPparams["imean"]
    print(a_, b_, c_)

    param_boundaries = [(-0.2, 0.4), (2.1, 2.5), (3.0, 3.5), (0.0, 0.06), (0.20, 0.45), (0.1, 0.25)]
