...
def FN_func(var_params, const_params, zobs, er, es, ei, lmin, lmax, smin):

    smean, imean = var_params
    a, b, c, sigma1, sigma2, sigma3 = const_params
...
def FP_func(var_params, const_params, logdists, z_obs, r, s, i, err_r, err_s, err_i, Sn, smin, lmin, lmax, C_m, sumgals=True, chi_squared_only=False, use_full_fn=True):
    ...
    # Calculate full f_n
    if use_full_fn:
        FN = FN_func(var_params, const_params, z_obs, err_r, err_s, err_i, lmin, lmax, smin) + np.log(C_m)
...
def fit_FP(
    ...
    while not is_converged:
        ...
        FPparams = sp.optimize.differential_evolution(FP_func, bounds=(svals, ivals), 
...
def fit_logdist(
    ...
    # Calculate full FN
    FNvals = FN_func(var_params, const_params, df["z_cmb"].to_numpy(), df["er"].to_numpy(), df["es"].to_numpy(), df["ei"].to_numpy(), lmin, lmax, smin)
...
def main():
    ...
    # Constant parameters
    const_params = np.array([a, b, c, sigma1, sigma2, sigma3])
    print("Constant parameters: ", const_params)

    param_boundaries = [(2.1, 2.5), (3.0, 3.5)]
    ...
    for survey in SURVEY_LIST:
        ...
        logdist_output_filepath = f"/Users/mrafifrbbn/Documents/thesis/thesis-research-2.0/experiments/experiment_024_vary_centroid/logdists/{survey.lower()}.csv"
    ...
    FP_columns = ['rmean', 'smean', 'imean']
    filepath_ = f"/Users/mrafifrbbn/Documents/thesis/thesis-research-2.0/experiments/experiment_024_vary_centroid/fp_fits.csv"
