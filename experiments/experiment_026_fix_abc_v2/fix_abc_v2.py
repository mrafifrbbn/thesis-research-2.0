...
def fit_FP(
    ...
    smin: float,
    param_boundaries: List[Tuple],
    reject_outliers: bool = REJECT_OUTLIERS
          ) -> Tuple[np.ndarray, pd.DataFrame]:
    
    print(f"{'=' * 10} Fitting 18-parameters Fundamental Plane")
    ...
    while not is_converged:
        ...
        FPparams = sp.optimize.differential_evolution(likelihood_function, bounds=(
            avals, bvals, cvals,
            svals, ivals, s1vals, s2vals, s3vals,
            svals, ivals, s1vals, s2vals, s3vals,
            svals, ivals, s1vals, s2vals, s3vals,
        ), args=(df_6df, df_sdss, df_lamost, smin), maxiter=10000, tol=1.0e-4, workers=-1, seed=42)
        ...
        # chi_squared = FP_func(FPparams.x, abc_params, 0.0, df["z_cmb"].to_numpy(), df["r_true"].to_numpy(), df["s"].to_numpy(), df["i"].to_numpy(), df["er"].to_numpy(), df["es"].to_numpy(), df["ei"].to_numpy(), Sn, smin, df["lmin"].to_numpy(), df["lmax"].to_numpy(), df["C_m"].to_numpy(), sumgals=False, chi_squared_only=True)[0]
...
def sample_likelihood(dfs: List[pd.DataFrame],
    ...
    # Load individual best-fit FP as estimates
    ...
    pos = FP_params + 1e-4 * np.random.randn(36, 18)
...
def main():
    try:
        ...
        # logger.info(f"Fitting the 18-parameter FP...")
        ...
        chain_output_filepath = "/Users/mrafifrbbn/Documents/thesis/thesis-research-2.0/experiments/experiment_026_fix_abc_v2/fp_fits/chain.npy"
        ...
        # logger.info("Saving the derived FP fits as an artifact...")
        # FP_columns = [
        #     "a", "b", "c",
        #     "smean_6df", "imean_6df", "sigma1_6df", "sigma2_6df", "sigma3_6df",
        #     "smean_sdss", "imean_sdss", "sigma1_sdss", "sigma2_sdss", "sigma3_sdss",
        #     "smean_lamost", "imean_lamost", "sigma1_lamost", "sigma2_lamost", "sigma3_lamost"
        # ]
        # filepath_ = "/Users/mrafifrbbn/Documents/thesis/thesis-research-2.0/experiments/experiment_026_fix_abc_v2/fp_fits.csv"
        # df = pd.DataFrame(params, columns=FP_columns)
        # df.to_csv(filepath_)
