...
def fit_mock(
    ...
    while not is_converged:
        ...
        chi_squared = FP_func(FPparams.x, 0.0, df["z"].to_numpy(), df["r_true"].to_numpy(), df["s"].to_numpy(), df["i"].to_numpy(), df["dr"].to_numpy(), df["ds"].to_numpy(), df["di"].to_numpy(), Sn, smin, df["lmin"].to_numpy(), df["lmax"].to_numpy(), df["C_m"].to_numpy(), sumgals=False, chi_squared_only=True, use_full_fn=use_full_fn)[0]
...
def fit_logdist(
    ...
    # Calculate log prior
    log_prior = -3 * dbins * np.log(10)
    log_prior_tiles = np.tile(log_prior, (FNvals.shape[1], 1)).T

    # Convert to the PDF for logdistance
    logP_dist = - loglike - FNvals + log_prior_tiles
    ...
    df[f'logdist_{mag_high}'] = logdist_mean
    df[f'logdist_{mag_high}_err'] = logdist_std
    df[f'logdist_{mag_high}_chisq'] = chisq
...
def main():
    ...
    # Load mock data
    input_path = os.path.join(ROOT_PATH, "experiments/experiment_036_mock_malmquist_prior/mock_full.txt")
    ...
    mag_lims = ["13.65", "13.15", "12.65", "12.15"]
    ...
    for mock_id in [1, 2, 3, 4, 5]:
        ...
        for mag_lim in mag_lims:
            ...
            chain_output_filepath = f"/Users/mrafifrbbn/Documents/thesis/thesis-research-2.0/experiments/experiment_036_mock_malmquist_prior/fp_fits/mock_{mock_id}/chain_{mag_lim}.npy"
            ...
            logdist_filepath = os.path.join(ROOT_PATH, f"experiments/experiment_036_mock_malmquist_prior/logdists/mock_{mock_id}/mock_{mag_lim}.csv")
            create_parent_folder(logdist_filepath)
            df_logdist.to_csv(logdist_filepath, index=False)

        # Save FP parameters for a single mock
        pd.DataFrame(fp_params_list).to_csv(f"/Users/mrafifrbbn/Documents/thesis/thesis-research-2.0/experiments/experiment_036_mock_malmquist_prior/fp_fits/mock_{mock_id}/best_fits.csv", index=False)
