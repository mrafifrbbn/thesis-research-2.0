...
def FP_func(params, logdists, z_obs, r, s, i, err_r, err_s, err_i, Sn, smin, lmin, lmax, sumgals=True, chi_squared_only=False):
...
def sample_likelihood(df, param_best_fits) -> None:
    ...
    pos = param_best_fits + 1e-4 * np.random.randn(16, 8)
    ...
    chain_filepath = os.path.join(ROOT_PATH, f'scrap/new_fp_method_finalized/{REDSHIFT_FN}/{survey.lower()}_chain.npy')
...
if __name__ == "__main__":
    ...
    # Load the data
    data_filepath = os.path.join(ROOT_PATH, f'data/foundation/fp_sample/smin_setting_1/{survey.lower()}.csv')
    ...
    d_H = sp.interpolate.splev(df[REDSHIFT_FN].to_numpy(), dist_spline, der=0)
    ...
    print(len(df))
    outlier_output_filepath = os.path.join(ROOT_PATH, f'scrap/new_fp_method_finalized/{REDSHIFT_FN}/{survey.lower()}.csv')
