...
def FP_func(params, logdists, z_obs, r, s, i, err_r, err_s, err_i, Sn, smin, lmin, lmax, sumgals=True, chi_squared_only=False):
    ...
    FN = 0. #FN_func(params, z_obs, err_r, err_s, err_i, lmin, lmax, smin)
    ...
    if chi_squared_only:
        ...
        # return 0.5 * np.sum(chi_squared + log_det)
    else:
...
def log_probability(theta, param_boundaries, logdists, z_obs, r, s, i, err_r, err_s, err_i, Sn, smin, lmin, lmax, sumgals=True, chi_squared_only=False):
...
def sample_likelihood(df, param_best_fits) -> None:
    ...
    sampler = emcee.EnsembleSampler(
        nwalkers, ndim, log_probability, args=(param_boundaries, 0., z, r, s, i, dr, ds, di, Sn, smin, lmin, lmax, True, False)
    )
    ...
    chain_filepath = os.path.join(ROOT_PATH, f'scrap/modified.npy')
