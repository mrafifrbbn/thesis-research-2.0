...
from main_code.section_4_logdist.step_2_distance_modulus import calculate_distance_modulus

from main_code.utils.constants import *
from main_code.utils.functions import *
from main_code.utils.CosmoFunc import *
from main_code.utils.filepaths import H22_FILEPATH, H22_REFIT_FILEPATH, H22_FP_FITS_FILEPATH
...
def fetch_h22_data(
        zmin: float = ZMIN,
        zmax: float = ZMAX,
        mag_lim: float = R_MAG_NEW_LIM,
) -> pd.DataFrame:

    # Load H22 data
    logger.info(f"Loading H22 data from {H22_FILEPATH}...")
    df = pd.read_csv(H22_FILEPATH, delim_whitespace=True)
    logger.info(f"Number of original data: {len(df)}")

    # Add 'SDSS' prefix to objid
    ...
    logger.info(f"Applying selections: m_lim = {mag_lim} | zmin = {zmin} | zmax = {zmax}")
    df = df[(df["deVMag_r"] - df["extinction_r"]) <= mag_lim]

    # Also apply the same redshift cut
    df = df[(df['zcmb_group'] >= zmin) & (df['zcmb_group'] <= zmax)]
    ...
    logger.info(f"Number of selected data: {len(df)}")
...
def fit_logdist(
    ...
    loglike = FP_func(FPparams, dbins, df["z_cmb"].to_numpy(), df["r"].to_numpy(), df["s"].to_numpy(), df["i"].to_numpy(), df["er"].to_numpy(), df["es"].to_numpy(), df["ei"].to_numpy(), np.ones(len(df)), smin, df["lmin"].to_numpy(), df["lmax"].to_numpy(), df["C_m"].to_numpy(), sumgals=False)
...
def main():
    ...
    fp_params_list = []
    for i, with_full_fn in enumerate([True, False]):
        # Fit FP
        logger.info(f"Fitting the FP with full_fn = {with_full_fn}")
        fp_params, df_clean = fit_FP(
            ...
            zmin=ZMIN,
            zmax=ZMAX,
            solar_magnitude=SOLAR_R_MAGNITUDE,
            mag_high=R_MAG_NEW_LIM,
            mag_low=MAG_LOW,
            reject_outliers=True,
            use_full_fn=with_full_fn
        )
        fp_params_list.append(fp_params)
        logger.info(f"Remaining data: {len(df_clean)}")
        logger.info(f"Best-fit params: {fp_params}")
        ...
        # Save data
        filepath = H22_REFIT_FILEPATH[f'fp_method_{i}']
        df_logdist.to_csv(filepath, index=False)

    FP_params = np.array(fp_params_list)
    FP_columns = ['a', 'b', 'rmean', 'smean', 'imean', 's1', 's2', 's3']
    df = pd.DataFrame(FP_params, columns=FP_columns, index=['full_fn', 'partial_fn'])
    df.to_csv(H22_FP_FITS_FILEPATH)
