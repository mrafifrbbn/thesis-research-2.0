...
def FP_func(params, abc_params, logdists, z_obs, r, s, i, err_r, err_s, err_i, Sn, smin, lmin, lmax, C_m, sumgals=True, chi_squared_only=False, use_full_fn=True):
    
    smean, imean, sigma1, sigma2, sigma3 = params
    a, b, c = abc_params
    ...
    # Calculate full f_n
    if use_full_fn:
        params_ = np.array([a, b, rmean, smean, imean, sigma1, sigma2, sigma3])
        FN = FN_func(params_, z_obs, err_r, err_s, err_i, lmin, lmax, smin) + np.log(C_m)
...
def fit_FP(
    ...
    while not is_converged:
        ...
        svals, ivals = param_boundaries[0], param_boundaries[1]
        s1vals, s2vals, s3vals = param_boundaries[2], param_boundaries[3], param_boundaries[4]

        # Fit the FP parameters
        FPparams = sp.optimize.differential_evolution(FP_func, bounds=(svals, ivals, s1vals, s2vals, s3vals), 
            args=(abc_params, 0.0, data_fit["z_cmb"].to_numpy(), data_fit["r_true"].to_numpy(), data_fit["s"].to_numpy(), data_fit["i"].to_numpy(), data_fit["er"].to_numpy(), data_fit["es"].to_numpy(), data_fit["ei"].to_numpy(), Snfit, smin, data_fit["lmin"].to_numpy(), data_fit["lmax"].to_numpy(), data_fit["C_m"].to_numpy(), True, False, use_full_fn), maxiter=10000, tol=1.0e-6, workers=-1, seed=42)
        ...
        chi_squared = Sn * FP_func(FPparams.x, abc_params, 0.0, df["z_cmb"].to_numpy(), df["r_true"].to_numpy(), df["s"].to_numpy(), df["i"].to_numpy(), df["er"].to_numpy(), df["es"].to_numpy(), df["ei"].to_numpy(), Sn, smin, df["lmin"].to_numpy(), df["lmax"].to_numpy(), df["C_m"].to_numpy(), sumgals=False, chi_squared_only=True)[0]
...
def sample_likelihood(df: pd.DataFrame,
    ...
    def log_prior(theta):
        smean, imean, sig1, sig2, sig3 = theta
        smean_bound, imean_bound, s1_bound, s2_bound, s3_bound = param_boundaries
        if smean_bound[0] < smean < smean_bound[1] and imean_bound[0] < imean < imean_bound[1] and s1_bound[0] < sig1 < s1_bound[1] and s2_bound[0] < sig2 < s2_bound[1] and s3_bound[0] < sig3 < s3_bound[1]:
            return 0.0
        else:
            return -np.inf

    # Calculate log-posterior distribution
    def log_probability(theta, abc_params, logdists, z_obs, r, s, i, err_r, err_s, err_i, Sn, smin, lmin, lmax, C_m, sumgals=True, chi_squared_only=False, use_full_fn=True):
        lp = log_prior(theta)
        if not np.isfinite(lp):
            return -np.inf
        else:
            return lp - FP_func(theta, abc_params, logdists, z_obs, r, s, i, err_r, err_s, err_i, Sn, smin, lmin, lmax, C_m, sumgals, chi_squared_only, use_full_fn)
    ...
    pos = FP_params + 1e-4 * np.random.randn(16, 5)
    nwalkers, ndim = pos.shape

    # Run the MCMC
    logger.info("Running the MCMC sampler")
    sampler = emcee.EnsembleSampler(
        nwalkers, ndim, log_probability, args=(abc_params, 0.0, z, r, s, i, dr, ds, di, Sn, smin, lmin, lmax, C_m, True, False, USE_FULL_FN)
    )
    ...
    # Calculate rmean and append to params
    flat_samples_T = flat_samples.T
    rmean = abc_params[2] + abc_params[0] * flat_samples_T[0] + abc_params[1] * flat_samples_T[1]
    flat_samples_T = np.insert(flat_samples_T, [0], rmean, axis=0)
...
#     """
#     print(f"Calculating the logdistance PDF for galaxies in {survey} | FP: {list(FPparams)}")
...
def main():
    try:
        ...
        FPparams["c"] = FPparams["rmean"] - FPparams["a"] * FPparams["smean"] - FPparams["b"] * FPparams["imean"]
        a, b, c = FPparams[["a", "b", "c"]]
        abc_params = [a, b, c]
        logger.info(f"Values of a, b, c: {abc_params}")

        # Set parameter boundaries
        param_boundaries = [(2.1, 2.5), (3.0, 3.5), (0.0, 0.06), (0.20, 0.45), (0.1, 0.25)]
        ...
        for survey in SURVEY_LIST:
            ...
            logger.info(f"Fitting the FP for {survey}. Setting the values of a, b, and c fixed...")
            ...
            # logger.info(f"FP_params_bestfit: {FP_params_bestfit}")

            # Save new galaxy sample
            filepath = OUTLIER_REJECT_FP_SAMPLE_ABC_FIXED_FILEPATHS[survey]
            create_parent_folder(filepath)
            df_fitted.to_csv(filepath, index=False)

            logger.info(f"Sampling the FP likelihood for {survey}.")
            chain_output_filepath = MCMC_CHAIN_ABC_FIXED_FILEPATHS[survey]
            ...
            # Calculate rmean and insert to params
            ...
            # logdist_output_filepath = f"/Users/mrafifrbbn/Documents/thesis/thesis-research-2.0/experiments/experiment_020_fix_abc/rmean_fixed/{survey.lower()}.csv"
        ...
        FP_columns = ['smean', 'imean', 's1', 's2', 's3']
        filepath_ = FP_FIT_ABC_FIXED_FILEPATH
        create_parent_folder(filepath_)
        df = pd.DataFrame(FP_params_bestfit, columns=FP_columns, index=SURVEY_LIST)
        df["a"] = a
        df["b"] = b
        df["c"] = c
        df["rmean"] = c + a * df["smean"] + b * df["imean"]
